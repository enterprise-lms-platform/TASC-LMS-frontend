...
const MobileFilterDrawer: React.FC<MobileFilterDrawerProps> = ({ open, onClose, filters, onFiltersChange, onClearAll }) => {
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <Box sx={{ p: 2, borderTop: '1px solid #e4e4e7', bgcolor: 'white' }}>
          <Stack direction="row" spacing={2}>
            <Button variant="outlined" fullWidth onClick={handleClearAll} sx={{ color: '#52525b', borderColor: '#d4d4d8' }}>Clear All</Button>
            <Button variant="contained" fullWidth onClick={handleShowResults} sx={{ bgcolor: '#ffa424', '&:hover': { bgcolor: '#f97316' } }}>Show Results</Button>
          </Stack>
        </Box>
      </Box>
    </Drawer>
