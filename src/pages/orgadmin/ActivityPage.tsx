...
const ActivityPage: React.FC = () => {
  ...
  const stats = [
    { label: 'Enrollments', value: summary?.enrollments ?? 0, color: '#6366f1' },
        ...
        <Box sx={{ p: { xs: 2, md: 3 } }}>
          ...
          <Box sx={{ display: 'flex', gap: 1, mb: 3 }}>
            {stats.map((stat) => (
              <Chip
                key={stat.label}
                icon={stat.label === 'Enrollments' ? <EnrollmentIcon /> : stat.label === 'Completions' ? <CompletionIcon /> : <SubmissionIcon />}
                ...
                <Table size="small">
                  <TableHead>
                    <TableRow>
                      ...
                      <TableCell sx={{ fontWeight: 600, color: 'text.secondary', fontSize: '0.75rem', textTransform: 'uppercase' }}>User Name</TableCell>
